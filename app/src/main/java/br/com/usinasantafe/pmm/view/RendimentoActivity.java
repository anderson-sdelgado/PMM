...
public class RendimentoActivity extends ActivityGeneric {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        LogProcessoDAO.getInstance().insert("int cont;\n" +
        ...
        LogProcessoDAO.getInstance().insert("rendMMBean =  pmmContext.getMotoMecFertCTR().getRend(cont);", getLocalClassName());
        rendMMBean =  pmmContext.getMotoMecFertCTR().getRend(cont);

        textViewRendimento.setText("OS " + rendMMBean.getNroOSRendMM() +" \nRENDIMENTO :");

        LogProcessoDAO.getInstance().insert("if(rendMMBean.getValorRendMM() > 0){\n" +
        ...
        buttonOkRendimento.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                LogProcessoDAO.getInstance().insert("buttonOkRendimento.setOnClickListener(new View.OnClickListener() {\n" +
                        "            @Override\n" +
        ...
        buttonCancRendimento.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                LogProcessoDAO.getInstance().insert("buttonCancRendimento.setOnClickListener(new View.OnClickListener() {\n" +
                        "            @Override\n" +
                        "            public void onClick(View v) {\n" +
    ...
    public void onBackPressed()  {
        ...
        if(pmmContext.getConfigCTR().getConfig().getPosicaoTela() == 4L){
            ...
            if(pmmContext.getMotoMecFertCTR().getPosRend() > 1){
                LogProcessoDAO.getInstance().insert("if(pmmContext.getMotoMecFertCTR().getPosRend() > 1){\n" +
    ...
    public void verTela(Double rendNum){
        ...
        LogProcessoDAO.getInstance().insert("rendMMBean.setValorRendMM(" + rendNum + ");\n" +
        ...
        if (pmmContext.getConfigCTR().getConfig().getPosicaoTela() == 4L) {
            ...
            if (pmmContext.getMotoMecFertCTR().qtdeRend() == pmmContext.getMotoMecFertCTR().getContRend()) {
                LogProcessoDAO.getInstance().insert("if (pmmContext.getMotoMecFertCTR().qtdeRend() == pmmContext.getMotoMecFertCTR().getContRend()) {\n" +
                        "                pmmContext.getMotoMecFertCTR().salvarBolMMFertFechado();", getLocalClassName());
                pmmContext.getMotoMecFertCTR().salvarBolMMFertFechado(getLocalClassName());
                LogProcessoDAO.getInstance().insert("Intent it = new Intent(RendimentoActivity.this, MenuInicialActivity.class);", getLocalClassName());
    ...
    public void verifRend(){
        LogProcessoDAO.getInstance().insert("String rend = editTextPadrao.getText().toString();\n" +
        ...
        if (rendNum <= pmmContext.getMotoMecFertCTR().rendOS(rendMMBean.getNroOSRendMM())) {
            LogProcessoDAO.getInstance().insert("if (rendNum <= pmmContext.getMotoMecFertCTR().rendOS(rendMMBean.getNroOSRendMM())) {\n" +
