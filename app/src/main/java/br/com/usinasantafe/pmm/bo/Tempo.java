...
public class Tempo {
    ...
    public void manipDataHora(String data){
        ...
        try{
            ...
            Date dataHora5maior = new Date(dataServ.getTime() + (6 * 60 * 1000));
            Date dataHora5menor = new Date(dataServ.getTime() - (6 * 60 * 1000));

            Date dataHoraCelular = new Date();
            Calendar celDataHoraCelular = Calendar.getInstance();
            celDataHoraCelular.setTime(dataHoraCelular);

            if(!(dataHoraCelular.before(dataHora5maior) && dataHoraCelular.after(dataHora5menor))){
                ManipDadosEnvio.getInstance().setStatusEnvio(4);
            }
